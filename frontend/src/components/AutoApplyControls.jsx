...
import {
  Sliders, Zap, Shield, Save, CheckCircle, AlertCircle, RefreshCw,
  Play, Pause, Target, BarChart2, Calendar, Clock, Layers, Filter
} from 'lucide-react';
...
function PlatformRuleCard({ item, onSave }) {
  ...
  const save = async () => {
    ...
    try {
      await request({
        ...
        data: { dailyLimit: Number(dailyLimit), targetRole, isEnabled },
      });
      setSavedMsg('Saved!');
      onSave?.();
...
export default function AutoApplyControls() {
  ...
  const fetchRules = useCallback(async () => {
    try {
      const res = await request({ method: 'get', url: '/automation/rules' });
      setRules(res.platformRules || []);
      setSummary(res.summary || { totalToday: 0, overallTotal: 0 });
    } catch (err) {
      console.error('Fetch automation rules failed:', err.message);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="max-w-4xl">
      ...
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">
            Application Rules & Daily Limits
          </h1>
          <p className="mt-2 max-w-xl text-sm leading-6 text-slate-400">
            Control automated job applications across all connected platforms. Set daily application limits (e.g. 25/day for Data Engineer roles) and monitor real-time activity.
          </p>
        </div>
        <button
          onClick={fetchRules}
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-white flex items-center gap-2">
          <Sliders size={15} className="text-aqua" /> Platform Automation Rules
        </h2>
        <span className="text-[11px] text-slate-400">Configure max applications per role</span>
      </div>
