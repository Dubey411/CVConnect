...
export class ResumeParser {
  ...
  async parse(file) {
    ...
    return {
      contact: { name: lines[0] || '', email, phone, location: lines.find(l => /(?:india|usa|uk|remote|,)/i.test(l)) || '' },
      summary: section(text, ['summary', 'profile', 'professional summary', 'objective'], 'experience|work history|education|projects|skills') || lines.slice(1, 4).join(' '),
